...
if __name__ == '__main__':
    ...
    if not opt.cover:
        file_no = 0
        name = opt.filepath
        while os.path.exists(opt.filepath):
            file_no += 1
            opt.filepath = name + '_' + str(file_no)
    if not os.path.exists(opt.filepath):
        os.makedirs(opt.filepath)
        ...
        for e in range(epoch):
            ...
            for i, (image, _) in enumerate(data_loader):
                if opt.dataset == 'mvtec':
                    # image = myMVTec.Fullcut(image)
                    # image = torch.cat(image)
                    image = myMVTec.Randomcut(image, size=(224, 224), patch=16)
                if cuda_availabel:
                    image = image.cuda()

                # experiment 2: Decoder only gets inputs from Teacher.
                ...
                print('', end='\r')
                print('Training Student: batch,', i + 1, '/', len(data_loader), '; epoch,', e + 1, '/', epoch,
                      '; loss,', 'Rec', Loss_S.item(), end='')


            # 训练一定次数验证一次，保留最好的model和anomalyscore
            if (e + 1) % opt.evalepoch == 0 or (e + 1) == epoch:
                ...
                #
                print('AUC: {}, for class {}, epoch: {}'.format(AUC, normal_lab, e + 1))
                result.append([normal_lab, e + 1, AUC])
                ...
                if AUC > bestAUC:
                    bestAUC = AUC
                    if opt.savemodel:
                        torch.save(Net_SEn, opt.filepath + '/Net_SEn' + str(normal_lab) + '.pkl')
                        torch.save(Net_SDe, opt.filepath + '/Net_SDe' + str(normal_lab) + '.pkl')
                        # torch.save(Net_Weight, opt.filepath + '/Net_Weight' + str(normal_lab) + '.pkl')
                    torch.save([AS_rec], opt.filepath + '/anomaly_score' + str(normal_lab) + '.data')
                    print('save the model and anomaly score, class:{}, epoch: {}, AUC: {}'.format(normal_lab, e+1, bestAUC))
    ...
    for normal_lab in range(15 if opt.dataset == 'mvtec' else 10):
        ...
        AS_rec = torch.load(opt.filepath + '/anomaly_score' + str(normal_lab) + '.data', map_location='cpu')
