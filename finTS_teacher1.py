    ...
    for normal_lab in range(opt.start, 15 if opt.dataset == 'mvtec' else 10):
        ...
        for e in range(epoch):
            ...
            for i, (image, _) in enumerate(data_loader):
                ...
                z_T = z1
                z_S = Net_SEn(image)
                Loss_SEn = MSE(z_T.detach(), z_S) + torch.mean(1 - CosSim(z_T.detach(), z_S))
                Loss_SEn.backward()
                Opt_SEn.step()

                loss_recT = MSE(image, x_T)
                loss_KD = MSE(z_T, z_S) + torch.mean(1 - CosSim(z_T, z_S))

                print('', end='\r')
                print('Training Student: batch,', i + 1, '/', len(data_loader), '; epoch,', e + 1, '/', epoch,
                      '; loss,', 'KD:', loss_KD.item(), 'TRec', loss_recT.item(), end='')
            ...
            if (e + 1) % opt.evalepoch == 0 or (e + 1) == epoch:
                ...
                if res_class.max() > bestAUC:
                    bestAUC = res_class.max()
                    if opt.savemodel:
